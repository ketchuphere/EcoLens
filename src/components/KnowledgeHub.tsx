...
import { BookOpen, Award, CheckCircle, XCircle, ChevronRight, HelpCircle, ArrowLeft, Sun, RefreshCw, Apple, Globe } from 'lucide-react';
...
export const KnowledgeHub: React.FC<KnowledgeHubProps> = ({
  ...
  const [answeredHistory, setAnsweredHistory] = useState<Record<string, boolean>>({});
  ...
  const handleAnswerSubmit = (q: QuizQuestion) => {
    ...
    if (correct) {
      ...
      setAnsweredHistory(prev => ({ ...prev, [q.id]: true }));
    } else {
      setAnsweredHistory(prev => ({ ...prev, [q.id]: false }));
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in" id="knowledge_hub_container">
      {/* Article Detail View Reader */}
      {activeArticle ? (
        <div className="bg-white border border-slate-200/70 rounded-[32px] p-8 shadow-xs space-y-6">
          <button
            onClick={() => setActiveArticle(null)}
            className="flex items-center gap-2 text-xs font-bold text-slate-600 hover:text-slate-900 border border-slate-200 rounded-xl py-2 px-4 bg-white cursor-pointer no-print"
          >
          ...
          <div className="space-y-2">
            <span className="text-[10px] font-extrabold uppercase tracking-widest bg-emerald-50 text-emerald-800 border border-emerald-100 py-1 px-3 rounded-full">
          ...
          <div className="space-y-4 text-xs text-slate-700 leading-relaxed border-t border-slate-150 pt-6 font-sans">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 bg-white border border-slate-200/70 rounded-[32px] p-8 shadow-xs space-y-6">
            <div>
              ...
              <p className="text-xs text-slate-400">Read professional scientific insights on home power grids, diet shares, and circular recycling</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {ARTICLES.map(article => {
                ...
                else if (article.category === 'recycling') { catColor = "bg-blue-50 border-blue-155 text-blue-700"; iconEl = <RefreshCw className="w-5 h-5" />; }
                  ...
                  >
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        <span className={`text-[10px] uppercase font-extrabold py-1 px-2.5 rounded-full border ${catColor}`}>
                          {article.category}
                        </span>
                    ...
                    <div className="pt-4 mt-4 border-t border-slate-100 flex justify-between items-center text-[10px] font-black text-emerald-700 uppercase tracking-wider">
              ...
              {quizFinished ? (
                <div className="py-8 text-center space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-450 font-bold font-sans">You answered {quizScore} of {QUIZ_QUESTIONS.length} correctly.</p>
                  </div>
                  ...
                  <button
                    ...
                    className="py-2.5 px-6 text-xs font-bold text-slate-800 bg-slate-100 border border-slate-200 hover:bg-slate-250 rounded-xl cursor-pointer"
                  >
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="space-y-4">
                        <div className="flex justify-between items-center">
                          <span className="text-[10px] font-extrabold text-emerald-700 uppercase tracking-widest">Question {currentQuestionIndex + 1} / {QUIZ_QUESTIONS.length}</span>
                          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{q.category}</span>
                        </div>

                        <h4 className="text-[13px] font-extrabold text-slate-800 leading-normal font-sans">
                          {q.question}
                        </h4>
                          ...
                          {q.options.map((opt, oIdx) => {
                            ...
                            return (
                              <button
                                ...
                                className={`w-full text-left p-3.5 rounded-2xl border text-xs transition-all flex items-start gap-2.5 cursor-pointer font-sans ${optStyle}`}
                                disabled={isAnswerSubmitted}
                              >
                                <span className="font-extrabold shrink-0 text-slate-400">{String.fromCharCode(65 + oIdx)}.</span>
                                <span>{opt}</span>
                              </button>
                            );
                          })}
                        ...
                        {isAnswerSubmitted ? (
                          <div className="space-y-3 animate-fade-in">
                            <div className={`p-4 rounded-2xl border text-[11px] font-sans leading-relaxed ${
                              selectedAnswer === q.correctAnswer
                                ? 'bg-emerald-50 border-emerald-250 text-emerald-900 font-semibold'
                                : 'bg-rose-50 border-rose-250 text-rose-900'
                            }`}>
                              <div className="flex items-center gap-1.5 font-bold mb-1 uppercase text-[10px] tracking-wider">
                                {selectedAnswer === q.correctAnswer ? (
                                  <>
                              ...
                              <span>{q.expertExplanation}</span>
                            </div>

                            <button
                              type="button"
                              onClick={handleNextQuizQuestion}
                              className="w-full py-3 px-5 text-xs font-bold text-white bg-emerald-600 rounded-2xl hover:bg-emerald-700 cursor-pointer text-center tracking-wider"
                            >
                          ...
                          <button
                            ...
                            className={`w-full py-3 px-5 text-xs font-bold text-white rounded-2xl transition-all tracking-wider ${
                              selectedAnswer !== null 
